...
function Mealplanner({ token }) {
  ...
  let dt = new Date()
  useEffect(() => {
    dispatch(getMealPlan(auth.user.$id, dt, token))
    user.selectedFood = {}
  }, [])
  ...
  const handleOnSave = () => {
    console.log(token)
    dispatch(createMealPlan(selectedFood, period, auth.user.$id, token))
    setTimeout(() => {
      dispatch(getMealPlan(auth.user.$id, dt, token))
    }, 2000)
    setSelectedFood({})
  ...
  const handleOnDelete = (periodIndex, mealIndex) => {
    dispatch(deleteMeal(auth.user.$id, dt, periodIndex, token))
    setTimeout(() => {
      dispatch(getMealPlan(auth.user.$id, dt, token))
    }, 2000)
  }
                    ...
                    {meal.length !== 0 &&
                      meal.map((item, index2) => {
                        ...
                        nutrients[index].sugar =
                          nutrients[index].sugar + Math.round(item.food.sugar_g)

                        nutrients[index].carbs =
                          nutrients[index].carbs +
                          Math.round(item.food.carbohydrates_total_g)

                        nutrients[index].protein =
                          nutrients[index].protein +
                          Math.round(item.food.protein_g)
                          ...
                          <div className="Card-mealplan">
                            <div id="food-img">
                              <img
                                onClick={() => {
                                  window.open(
                                    `https://www.youtube.com/results?search_query=${item.food.name} reciepe`
                                  )
                                }}
                                src={item.food.image}
                                alt=""
                            ...
                            <div id="food-info">
                              ...
                              <div>
                                Serving Size:{' '}
                                {Math.round(item.food.serving_size_g)}
                              </div>
                              <div>
                                Calories: {Math.round(item.food.calories)}
                              </div>
                              <div>
                                Protein: {Math.round(item.food.protein_g)}
                              </div>
                              <div>
                                Carbs:
                                {Math.round(item.food.carbohydrates_total_g)}
                              </div>
                              <div>Sugar: {Math.round(item.food.sugar_g)}</div>
                              <div>
                                Quantity: {Math.round(item.food.quantity)}
                              </div>
                            </div>
                          </div>
                        )
